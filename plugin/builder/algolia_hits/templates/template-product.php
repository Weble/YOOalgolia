...
<ais-hits>
    ...
    <div v-for="item in items">
        ...
        <a :href="item.url['<?= $lang; ?>'].default" class="uk-card uk-card-small uk-card-default uk-card-hover uk-flex-1 uk-flex uk-flex-column">
            ...
            <div class="uk-card-media-top uk-padding uk-text-center uk-flex uk-flex-middle uk-flex-center uk-position-relative" v-if="item.image && item.image.length > 0">
                ...
                <div v-if="item.category && item.category.path.length > 1 && item.category.path[1].image" class="uk-position-absolute uk-position-top-left uk-padding-small">
                    <img :src="item.categories[0].path[1].image" width="20" alt="Metronet" />
                </div>

                <div v-if="item.tags && item.tags.indexOf('Metronet') >= 0" class="uk-position-absolute uk-position-top-right uk-padding-small">
                    <img src="/images/loghi/logo-metronet-small.png" width="20" alt="Metronet" />
                </div>

                <img style="max-height: 150px;" :src="item.image[0]" @error="item.image = 'images/prodotti/product-placeholder.jpg'" height="150" />
